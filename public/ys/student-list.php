<?php
	...
	$sql="select * from y_user where isStudent = 1";
	$list = selectAll($sql);
?>
<!DOCTYPE HTML>
<html>
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
			<thead>
				<tr class="text-c">
					...
					<th width="80">用户名</th>
					<th width="60">头像</th>
					<!--<th width="80">剩余雅思课卡</th>
					<th width="80">剩余情景英语课卡</th>
					<th width="80">用户姓名</th>
					<th width="80">联系方式</th>
					<th width="80">用户等级</th>
					<th width="80">班级</th>
			...
			<tbody>
				...
				<tr class="text-c">
					...
					<!--<td><?php echo $user["lesIelts"]; ?></td>
					<td><?php echo $user["lesEcourse"]; ?></td>
					<td><?php if($user["real_name"]){ echo $user["real_name"];}else{ echo "暂无";} ?></td>
					<td><?php if($user["tel"]){ echo $user["tel"];}else{ echo "暂无";} ?></td>
					<td><?php if($user["lv"]){switch($user["lv"]){case 0: echo "暂无";break; case 1: echo "小班";break; case 2: echo "vip";break;}}else{ echo "暂无";} ?></td>
					<td><?php if($user["user_class"]){$sql2 = "select * from s_class where class_id = ".$user["user_class"];$query = selectRow($sql2);echo $query["class_name"];}else{echo "暂无";} ?></td>
					<td><?php if($user["sign_time"]){ echo date("Y-m-d H:i:s",$user["sign_time"]);}else{ echo "暂无";} ?></td>
					<td><?php if($user["class_time"]){ echo date("Y-m-d H:i:s",$user["class_time"]);}else{ echo "暂无";} ?></td>
					<td class="f-14">
						<a style="text-decoration:none" class="ml-5" data-title="查看课程" data-href="user-used-list.php?id=<?php echo $user["uid"]; ?>" onclick="Hui_admin_tab(this)" href="javascript:;">查看课程</a>
					</td>-->
