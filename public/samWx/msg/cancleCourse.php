...
//获取access_token
     ...
     function send($openId,$formId,$token,$courseInfo)
    {
        ...
        $post_faqi = array(
            ...
            "template_id" =>'pzfoiT1AFmFUL46b1QUfJao7tA00s4Mamac_XXVprBU',//模板id
            "page" => 'pages/index/index',//跳转路径
            "form_id" => $formId,//form_id
            //data 自己根据公众平台申请的消息模板进行填写
            "data" => array(
                'keyword1' => array("value" => $courseName, "color" => "#9b9b9b"),
                'keyword2' => array("value" => $courseDate."  ".$courseTime, "color" => "#9b9b9b") )
...
if(isset($_GET['uid'])&&isset($_GET['cid'])&&isset($_GET['token']))
{
	 ...
	 if($user){
			   ...
			   $courseInfoSql = "SELECT * FROM  s_course_info WHERE info_id =".$cid;
